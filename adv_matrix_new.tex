...
\title[Matrix Identities Involving Multiplication and Transposition]{Matrix Identities Involving Multiplication\\ and Transposition}
...
Algebraic structures used in this paper are mostly of types $(2,1)$ or $(2,1,1)$ so
...
$(u)^*$.
\end{itemize}
The algebraic structure $\mathcal{T}(X)=\langle T(X),\cdot,{}^*\rangle$ of type $(2,1)$
whose binary operation $\cdot$ is concatenation and whose unary operation
is $u\mapsto(u)^*$ is called the \emph{free unary \sm} over $X$.
A \emph{unary \sm\ identity} over $X$ is merely an at most 2-element
subset $\{u,v\}\subset T(X)^+$ usually written as $u=v$. A unary \sm\
$\mathcal{S}=\langle S,\cdot,{}^*\rangle$ \emph{satisfies} the identity
$u=v$ if the equality $\varphi(u)=\varphi(v)$ holds in $\mathcal{S}$ under all
possible homomorphisms $\varphi:\mathcal{T}(X)\to\mathcal{S}$. Given $\mathcal{S}$,
...
$\Sigma$ of unary \sm\ identities is called the \emph{variety defined by $\Sc$}.
A variety is is said to be \emph{\fb} if it can be defined by a finite set
of identities; otherwise it is called \emph{\nfb}.

It is easy to see that identities are inherited by forming direct products,
taking unary subsemigroups and homomorphic images so that each variety is
closed under these operators. In fact, varieties can be characterized by
this closure property (the HSP-theorem, see \cite[Theorem 11.9]{BuSa81}).
...
the variety it generates are simultaneously \fb\ or \nfb.
...
finitely based locally finite variety. Since the variety generated
by a finite unary \sm\ is locally finite (this is an easy consequence
of the HSP-theorem, see \cite[Theorem 10.16]{BuSa81}), the property
of being inherently non-finitely based implies the property of being
...
\cite[Section~3]{GePe85} for more detail on $\FI(X)$.
...
$N(A)=\{x\in K^n\mid xA=0\}$ has dimension $n-1$ whence the root subspace of
...
immediately obtain
\begin{Cor}
\label{identity for rank 1} For any field $\mathcal{K}$, the semigroup
...
non-finitely based semigroups} Here we prove a sufficient condition for an involutory semigroup to be
inherently non-finitely based and exhibit two concrete examples of involutory semigroups
...
Before passing to concrete examples of inherently non-finitely
based involutory semigroups, we formulate a corollary of our proof
...
$\mathcal{V}_k^0$ satisfies the identity $x^2=0$. This identity is
...
$Z_n'\mathrel{\Rc}Z_n$, hence it satisfies the
quasi-identity
\begin{equation}
\label{quasidentity}
...
generated if and only if so is the semigroup $\mathcal{T}=\langle T,\cdot\rangle$.
...
variety generated by the semigroup $\langle S,\cdot\rangle$. Let
...
semigroup $\langle T,\cdot\rangle$ satisfies \eqref{quasidentity},
and therefore, it is finite provided that all its subgroups are locally finite.
Each group $\mathcal{H}$ that satisfies the identities \eqref{idempotent law}
and \eqref{crucial law} satisfies the identity $v(x_1,\dots,x_m)=1$, whence
$\mathcal{H}$ belongs to $\var\mathcal{G}$ and so is locally finite. Altogether,
...
isomorphic to the free monogenic involutory semigroup $\FI(\{\zeta\})$. The latter semigroup is known to
contain as a unary subsemigroup a free involutory semigroup on countably many generators, namely, $\FI(Z)$
where
...
inherently non-\fb\ (as a plain \sm) for every
...
$m\times m$-matrices over any field $\mathcal{K}$, the
...
inherently non-finitely based for each $m\ge 1$ and each finite
...
embedded as an involutory semigroup into $\Bc_n$ for each $n$,
the result follows.
\end{proof}
...
semiring is inherently non-finitely based whenever
...
with respect to the secondary diagonal is inherently non-finitely based.
\end{Thm}
...
One easily checks that this bijection extends to an isomorphism
between the Rees quotient $\mathcal{M}/\mathcal{N}$ and the \sm\
$\TA$; moreover, this isomorphism respects the unary operation.
We see that $\TA$ turns out to be an image of an involutory
sub\sm\ in $\BT_n$ under an involutary \sm\ homomorphism whence
$\TA$ belongs to the involutary \sm\ variety generated by $\BT_n$.
...
is inherently non-finitely based for $n\ge4$. However, the construction
...
the \sm\ $\BR_n=\langle BR_n,\cdot,{}^T\rangle$ of all Boolean
...
\begin{Problem}
Are the involutary semigroups $\BR_n$ and $\BU_n$ finitely based for $n\ge 3$?
\end{Problem}
The involutary semigroups $\BR_2$ and $\BU_2$ are easily seen to be
finitely based. The finite basis problem for the plain semigroups
$\langle BR_n,\cdot\rangle$ and $\langle BU_n,\cdot\rangle$ has been
solved by the third author~\cite{Vo04}.

\medskip

 \noindent\textbf{Acknowledgement.} The second author was
supported by Grant No.\ 144011 of the Ministry of Science and
Technological Development of the Republic of Serbia. The third
author acknowledges support from the Federal Education Agency of
Russia, grant 2.1.1/3537, and from the Russian Foundation for
Basic Research, grants 09-01-12142 and 10-01-00524.

\begin{thebibliography}{99}
\bibitem{Ami74}
S.\,A.\,Amitsur, \emph{Polynomial identities}, Israel J. Math \textbf{19}
(1974), 183--199.
...
Proceedings of the Conference on Semigroups with Applications,
J.\,M.\,Howie, W.\,D.\,Munn and H.-J.\,Weinert (eds.), World Scientific,
Singapore, 1992.
...
\bibitem{CoKo04}
J.\,Colombo and P.\,Koshlukov, \emph{Central polynomials in the matrix
algebra of order two}, Linear Algebra and its Applications \textbf{377}
(2004), 53--67.

\bibitem{DaRa99}
A.\,D'Amour and M.\,Racine, *-\emph{Polynomial identites of matrices with the
transpose involution: the low degrees}, Trans.\ Amer.\ Math.\ Soc. \textbf{351}
(1999), 5089--5106.

\bibitem{DaRa04}
A.\,D'Amour and M.\,Racine, *-\emph{Polynomial identites of matrices with thes
ymplectic involution: the low degrees}, Commun.\ Algebra \textbf{32} (2004),
895–-918.
...
\bibitem{Dr81}
V.\,S.\,Drensky, \emph{A minimal basis of identities for a second-order
matrix algebra over a field of characteristic $0$}, Algebra i Logika
\textbf{20} (1981), 282--290 [Russian;  Engl.\ translation Algebra
and Logic \textbf{20} (1982), 188--194].

\bibitem{DrFo04}
V.\,S.\,Drensky and E.\,Formanek, \emph{Polynomial Identity Rings},
Birkh\"auser, Basel, 2004.

\bibitem{DrGi95}
V.\,S.\,Drensky and A. Giambruno, \emph{On the *-polynomial identities
of minimal degree for matrices with involution}, Boll.\ Unione Math.\ Ital.
Ser.\,A, \textbf{9} (1995), 471--482.
...
\bibitem{Ge81}
G.\,K.\,Genov, \emph{A basis of identities of the algebra of third-order
matrices over a finite field}, Algebra i Logika \textbf{20} (1981), 385--388
[Russian;  Engl.\ translation Algebra and Logic \textbf{20} (1982), 241--257].

\bibitem{GeSi82}
G.\,K.\,Genov and P.\,N.\,Siderov, \emph{A basis of the identities of the fourth
order matrix algebra over a finite field}. I,II,  Serdica \textbf{8} (1982),
313--323, 351--366 [Russian].

\bibitem{GePe85}
J.\,A.\,Gerhard, M.\,Petrich, \emph{Free involutorial completely simple semigroups},
Canad. J. Math. \textbf{37} (1985), 271--295.

\bibitem{Gi90}
A.\,Giambruno, \emph{On *-polynomial identities for $n\times n$-matrices}, J. Algebra
\textbf{133} (1990), 433-–438.

\bibitem{GiZa05}
A.\,Giambruno and M.\,Zaicev, \emph{Polynomial Identities and Asymptotic
Methods}, Amer.\ Math.\ Soc., Providence, 2005.
...
\bibitem{KaRo05}
A.\,Kanel-Belov and L.\,H.\,Rowen, \emph{Computational Aspects of Polynomial
Identities}, A~K~Peters Ltd., Wellesley, 2005.

\bibitem{Ke87}
A.\,R.\,Kemer, \emph{The finite basis property of identities of associative
algebras}, Algebra i Logika \textbf{26} (1987), 597--641 [Russian; Engl.\
translation Algebra and Logic \textbf{26} (1987), 362--397].

\bibitem{Ke91}
A.\,R.\,Kemer, \emph{Ideals of Identities of Associative Algebras},
...
[Russian; English transl.\ Sib.\ Math. J. \textbf{20} (1979),
530--543].

\bibitem{Ko01}
P.\,Koshlukov, \emph{Basis of the identities of the matrix algebra
of order two over a field of characteristic $p\ne2$}, J. Algebra
\textbf{241} (2001), 410--434.

\bibitem{Kr73}
R.\,L.\,Kruse, \emph{ Identities satisfied by a finite ring}, J. Algebra
\textbf{26} (1973), 298--318.

\bibitem{LL}
Jian Rong Li and Yan Feng Luo, \emph{On the finite basis problem for
the monoids of triangular boolean matrices}, submitted.
...
\bibitem{Lv73}
I.\,V.\,L'vov, \emph{Varieties of associative rings}. I,
Algebra i Logika \textbf{12} (1973), 269--297 [Russian; Engl.\
translation Algebra and Logic \textbf{12} (1973), 150--167].
...
\bibitem{MaKu78}
Yu.\,N.\,Mal’tsev and E.\,N.\,Kuz’min, \emph{A basis for the identities
of the algebra of second-order matrices over a finite field}, Algebra i Logika
\textbf{17} (1978), 28--32 [Russian; English transl.\ Algebra and Logic
\textbf{17} (1978), 18--21].
...
\bibitem{PiSt85}
J-E. Pin and H. Straubing, \emph{Monoids of upper triangular matrices},
pp.259--272 in: Semigroups. Structure and Universal Algebraic Problems,
G. Poll\'ak, \v{S}t.\ Schwarz and O. Steinfeld (eds) [Colloquia
Mathematica Societatis J\'anos Bolyai \textbf{39}], North-Holland,
...
\bibitem{Ra73}
Yu.\,P.\,Razmyslov, \emph{Finite basing of the identities of a matrix
algebra of second order over a field of characteristic zero}, Algebra
i Logika \textbf{12} (1973), 83--113 [Russian; English transl.\
Algebra and Logic \textbf{12} (1973), 47--63].

\bibitem{Ro80}
L. H. Rowen, \emph{Polynomial Identities in Ring Theory}, Academic Press,
New York--London, 1980.

\bibitem{sapirinherently}
M.\,V.\,Sapir, \emph{Inherently non-finitely based finite
semigroups}, Mat.\ Sb. \textbf{133} (1987), 154--166 [Russian;
English transl.\ Math. USSR-Sb. \textbf{61} (1988), 155--166].
...
\bibitem{Vo04}
M.\,V.\,Volkov, \emph{Reflexive relations, extensive transformations
and piecewise testable languages of a given height}, Int.\ J. Algebra
and Computation \textbf{14} (2004), 817--827.

\bibitem{VoGo04}
M.\,V.\,Volkov, I.\,A.\,Goldberg, \emph{The finite basis problem
for monoids of triangular boolean matrices}, pp.205--214 in Algebraic
Systems, Formal Languages, and Conventional and Unconventional Computation
Theory [Surikaisekikenkyusho Kokyuroku \textbf{1366}], Research Institute
for Mathematical Sciences, Kyoto University, Kyoto, 2004.
\end{thebibliography}
