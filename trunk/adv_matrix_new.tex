...
But $\eta=\zeta^T$ whence the involutary subsemigroup in $\langle\mathrm{SL}_2(\bb Z),\cdot,{}^T\rangle$
...
\begin{Rmk}
It is easy to see that for $m>1$ the involutory semigroup
$\langle\mathrm{M}_{2m}(\mathcal{K}),\cdot,{}^S\rangle$ is finitely
based for any field $\mathcal{K}$ of characteristic $0$. (This follows
from the observation that the involutary subsemigroup in $\langle\mathrm{M}_4(\bb Z),\cdot,{}^S\rangle$
generated by the matrix $\begin{pmatrix} \zeta & O_2\\ O_2 & \zeta\end{pmatrix}$ where
$\zeta=\begin{pmatrix} 1 & 2\\ 0 & 1\end{pmatrix}$ is isomorphic to the free monogenic
involutory semigroup---compare with the argument in the beginning of
Subsection~\ref{usual transposition}.) However, the $2\times 2$-matrices
over a field of characteristic $0$ fulfill non-trivial identities
involving multiplication and the symplectic transpose (for instance,
$xx^Sy=yxx^S$ or $xx^S=x^Sx$), and we do not know whether or not the
