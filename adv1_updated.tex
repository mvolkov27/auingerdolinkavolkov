...
$(1,1,\ldots,1,2,2,\ldots,2,\ldots,r,r,\ldots,r)$ where each number occurs $r$
times. All even columns of $M$ are identical and equal to the transpose of the
row $(1,2,\ldots,r,\ldots,1,2,\ldots,r)$ in which the block $1,2,\dots,r$
occurs $r$ times.
...
\begin{Lemma}\label{lemma:disjoint} For all $m,n\ge 1$, the sets $\mathrm{Asc}A^n$ and
$\mathrm{Desc}B^m$ are disjoint.
\end{Lemma}
...
In particular, for any $C=(c_{ij})\in\mathrm{Asc}A^n$ we get
...
$$\deg(d)<\deg(dq)=
\deg(dq+bp).$$  Again, this shows that for any $D=(d_{ij})\in\mathrm{Desc}B^m$,
\begin{equation}\label{B}
\deg(d_{11})<\deg(d_{12}).
\end{equation}
Conditions (\ref{A}) and (\ref{B}) immediately imply that $\mathrm{Asc}A^n\cap
\mathrm{Desc}B^m=\varnothing$.
\end{proof}
...
because otherwise $A$ and $B$ would be invertible over the polynomial ring
...
\eqref{eqII} $u(A,B)\in \mathrm{Asc}A^{n_s}$ and
$v(A,B)\in\mathrm{Desc}B^{k_t}$. In any case, using Lemma \ref{lemma:disjoint}
...
Therefore we only need verifying that $\mathcal{GL}_2(\mathcal{K})$ satisfies
